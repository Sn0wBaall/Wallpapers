  ...
  if [ "$option" == "e" ]; then
    ...
    echo -e "\n${yellowColour}[!]${endColour}${grayColour} Showing all wallpapers${endColour}"
     for file in $(ls -v wall-*.jpg); do
       tput civis
      echo -e "${greenColour}[+]${endColour}${grayColour} Showing the wallpaper${endColour}${turquoiseColour} -->${endColour}${purpleColour} $file${endColour}"
      sleep 0.5
      kitten icat "$file"
      tput cnorm
    done
    ...
    for ((i = $read1; i <= $read2; i++)); do
      ...
      file="wall-$num.jpg"
      echo -e "\n${greenColour}[+]${endColour}${grayColour} Showing the wallpaper${endColour}${turquoiseColour} -->${endColour}${purpleColour} $file${endColour}"
      sleep 0.5
      kitten icat $file
      tput cnorm
    done
